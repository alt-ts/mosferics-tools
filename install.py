...
GITHUB_RAW_URL = "https://raw.githubusercontent.com/YOUR_USERNAME/mosferics-tools/main/generate_price_list.py"
...
def download_script(url, dest):
    ...
    import ssl
    # Try standard SSL first, fall back to unverified if corporate proxy blocks it
    for verified in [True, False]:
        try:
            ...
            if not verified:
                log("Note: downloaded without SSL verification (network proxy detected).")
            log(f"Downloaded to {dest}")
            return True
        except urllib.error.URLError as e:
            if verified:
                log(f"SSL verification failed, retrying without verification...")
                continue
